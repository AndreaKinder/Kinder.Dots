...
# @raycast.title Exportar tareas de la nota diaria
# @raycast.mode compact
# @raycast.packageName Notas diarias
# @raycast.description Exporta las tareas pendientes de la nota de hoy a Apple Reminders

JOURNAL_DIR="/Volumes/Files/notes/journal"
NOTE="$JOURNAL_DIR/$(date +%Y-%m-%d).md"

if [ ! -f "$NOTE" ]; then
  echo "No existe la nota diaria de hoy: $NOTE" >&2
  exit 1
fi
